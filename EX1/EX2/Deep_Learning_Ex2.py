...
def file_to_tensor(path, vocab):
    ...
    ids = [vocab[w] if w in vocab else vocab.get('<unk>', 0) for w in words]
...
class RNNModel(nn.Module):
    def __init__(self, rnn_type, vocab_size, emb_size=200, hidden_size=200, dropout=0.0):
        super().__init__()
        self.encoder = nn.Embedding(vocab_size, emb_size)
        self.rnn_type = rnn_type.lower()
        if self.rnn_type == 'lstm':
            self.rnn = nn.LSTM(emb_size, hidden_size, num_layers=1, dropout=0.0)
        elif self.rnn_type == 'gru':
            self.rnn = nn.GRU(emb_size, hidden_size, num_layers=1, dropout=0.0)
        else:
...
def evaluate(model, data_source, criterion, bptt, eval_batch_size):
    model.eval()
    total_loss = 0.0
...
# -----------------------
def plot_curves(results_dir, label, train_vals, valid_vals):
    # save as png; user can also view tensorboard
    plt.figure()
    epochs = range(1, len(train_vals)+1)
    plt.plot(epochs, train_vals)
    plt.plot(epochs, valid_vals)
    plt.xlabel('Epoch')
...
def run_experiment(args):
    ...
    # 2) model
    model = RNNModel(args.model, vocab_size, emb_size=200, hidden_size=200, dropout=args.dropout).to(device)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        val_loss = evaluate(model, valid_data, criterion, args.bptt, args.eval_batch_size)
        ...
        print(f"Epoch {epoch:02d} | Train ppl {train_ppl:.2f} | Val ppl {val_ppl:.2f} | Time {elapsed:.1f}s")
    ...
    # evaluate on train (smaller batch for evaluation)
    train_loss_final = evaluate(model, train_data, criterion, args.bptt, args.eval_batch_size)
    val_loss_final = evaluate(model, valid_data, criterion, args.bptt, args.eval_batch_size)
    test_loss_final = evaluate(model, test_data, criterion, args.bptt, args.eval_batch_size)
    ...
    plot_curves(results_dir, run_name, train_ppls, valid_ppls)
...
def parse_args():
    ...
    p.add_argument('--nonmono', type=int, default=6, help='start LR decay after this epoch (SGD)')
    p.add_argument('--lr_decay', type=float, default=0.5, help='LR decay factor')
    return p.parse_args()
